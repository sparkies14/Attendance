...
async function fetchAttentionData() {
  ...
  const result = (members || []).map(m => {
    const mEmail   = (m.email || '').toLowerCase();
    const attMember = attRows.filter(r => (r.email || '').toLowerCase() === mEmail);
    const minor    = attMember.filter(r => r.late_status === 'MINOR TARDY').length;
    const major    = attMember.filter(r => r.late_status === 'MAJOR TARDY').length;
    const awolHalf = attMember.filter(r => r.late_status === 'AWOL HALF DAY').length;
    const awolFull = attMember.filter(r => r.late_status === 'AWOL FULL DAY').length;
    const tardyCount = minor + major + awolHalf + awolFull;
    const discRecs = discRows.filter(r => r.user_id === m.id);
    const hasActiveWarning = discRecs.some(r => r.voided === false);

    const reasons = [];
    if (tardyCount >= 2) reasons.push('2+ tardies this month');
    if (hasActiveWarning) reasons.push('Active warning');
    if (reasons.length > 0) return { name: m.name, email: m.email, reasons };
    return null;
  });

  return { members: result.filter(Boolean) };
}
